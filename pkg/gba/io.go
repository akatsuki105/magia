...
func (g *GBA) getRAM32(addr uint32, s bool) uint32 {
	g.timers.Tick(g.waitBus(addr, 32, s))
...
func (g *GBA) getRAM16(addr uint32, s bool) uint32 {
	g.timers.Tick(g.waitBus(addr, 16, s))
	val := g._getRAM(addr)
	return val & 0x0000_ffff
}

func (g *GBA) getRAM8(addr uint32, s bool) byte {
	g.timers.Tick(g.waitBus(addr, 8, s))
...
func (g *GBA) _setRAM(addr uint32, val uint32, width int) {
	...
	case timer.IsIO(addr):
		switch width {
		case 1:
			g.timers.Set8(addr, byte(val))
		case 2:
			g.timers.Set16(addr, uint16(val))
		case 4:
