...
func (g *GBA) armALUOp2(inst uint32) uint32 {
	if !util.Bit(inst, 25) {
		...
		case lsl:
			return g.armLSL(g.R[rm]+salt, is)
		case lsr:
			return g.armLSR(g.R[rm]+salt, is)
		case asr:
			return g.armASR(g.R[rm]+salt, is)
		case ror:
			return g.armROR(g.R[rm]+salt, is)
		}
	}
