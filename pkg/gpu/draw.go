...
func (g *GPU) drawLayer(screen *image.RGBA, bgCnt uint16) {
	...
	for yTile := uint32(0); yTile < 32; yTile++ {
		for xTile := uint32(0); xTile < 32; xTile++ {
			mapData := util.LE16(g.VRAM[(mapBlock + 2*(yTile*32+xTile)):])
			...
			if colorMode == color16 {
				...
				for y := uint32(0); y < 8; y++ {
					for x := uint32(0); x < 4; x++ {
						c := tileData[y*8+x*2]
						screen.Set(int(xTile*8+x*2), int(yTile*8+y), g.paletteColor(paletteIdx, int(c&0b1111)))
						screen.Set(int(xTile*8+x*2+1), int(yTile*8+y), g.paletteColor(paletteIdx, int((c>>4)&0b1111)))
					}
				}
			} else {
				...
				for y := uint32(0); y < 8; y++ {
					for x := uint32(0); x < 8; x++ {
						screen.Set(int(xTile*8+x*2), int(yTile*8+y), g.paletteColor(-1, int(tileData[y*8+x])))
					}
				}
			}
		}
...
func (g *GPU) draw3() *image.RGBA {
	...
	for y := 0; y < 160; y++ {
		for x := 0; x < 480; x += 2 {
			...
			result.Set(x/2, y, color.RGBA{r, g, b, 0xff})
		}
	}
...
func (g *GPU) paletteColor(palette, index int) color.RGBA {
	...
	return color.RGBA{byte((c & 0b11111) * 8), byte(((c >> 5) & 0b11111) * 8), byte(((c >> 10) & 0b11111) * 8), 0xff}
}
